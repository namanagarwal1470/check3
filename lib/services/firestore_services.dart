...
class FirestoreServices {
  Future<List<MathQuestion>> fetch_data() async {
    CollectionReference math_questions =
        await FirebaseFirestore.instance.collection('Mathematics');
    CollectionReference passage_questions =
        await FirebaseFirestore.instance.collection('Passage based Questions');
  ...
  List<MathQuestion> assemble_data(
      List<DocumentSnapshot> math_docs, List<DocumentSnapshot> passage_docs) {
    List<MathQuestion> questions = [];
    try {
      questions = math_docs
          .map((e) => MathQuestion.fromMap(
              e.data() as Map<String, dynamic>, passage_docs))
          .toList();
    } catch (e) {
      print(e.toString());
    }
